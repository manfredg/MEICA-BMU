...
def selcomps(seldict,debug=False,olevel=1,oversion=99):
	...
	Rhos_elbow = min([Rhos_lim[getelbow(Rhos_lim)]  , Rhos[getelbow(Rhos)], getfbounds(ne)[0]])
	...
	if len(ncl)>len(good_guess):
		...
		#Removal of high Rho and ultra-high variance stuff
		#Rhos_soft_elbow = np.mean([Rhos_lim[getelbow(Rhos_lim)], Rhos[getelbow(Rhos)], getfbounds(ne)[0]])
		#new_guess = ncl[andb([Kappas[ncl]>=Kappas_elbow,Rhos[ncl]<=Rhos_soft_elbow])==2]

...
def writect(comptable,ctname='',classarr=[]):
	...
	with open(ctname,'w') as f:
		...
		f.write("##ACC %s \t#Accepted BOLD components\n" % ','.join([str(int(cc)) for cc in acc]) )
		...
		f.write("##DFn %i \t#Nominal denoised time series degrees of freedom\n" %  (nt-len(midk)-len(rej)) )
		f.write("##DFe %i \t#Effective degrees of freedom for correlation (lower bound)\n" %  ( len(acc) ) )

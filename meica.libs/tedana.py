...
def writect(comptable,ctname='',varexpl='-1',classarr=[]):
	...
	with open(ctname,'w') as f:
		...
		f.write("##VEx %.02f \t#Dataset variance explained by ICA\n" %  ( varexpl ) )
		f.write("##TCo %i \t#Total components generated by decomposition\n" %  ( nc ) )
		f.write("##DFe %i \t#No. accepted BOLD-like components, i.e. effective degrees of freedom for correlation (lower bound)\n" %  ( len(acc) ) )
		f.write("##RJn %i \t#Total number of rejected components\n" %  (len(midk)+len(rej)) )
		f.write("##DFn %i \t#Nominal degress of freedom in denoised time series (..._medn.nii.gz) \n" %  (nt-len(midk)-len(rej)) )
